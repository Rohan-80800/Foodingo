...
export const placeOrder = async (req, res) => {
  try {
    ...
    if (
      (!deliveryAddress.text || !deliveryAddress.latitude,
  ...
  try {
    ...
    if (status == "out of delivery" && !shopOrder.assignment) {
      ...
      deliveryBoysPayload = availableBoys.map((b) => ({
        ...
        latitude: b.location.coordinate?.[1],
        mobile: b.mobile
      }));
    }
...
export const sentDeliveryOtp = async (req, res) => {
  try {
    ...
    if (!order || !shopOrder) {
      return res
        .status(400)
        .json({ messgae: "Enter valid order/shopOrder id" });
    }
    ...
    return res
      .status(500)
      .json({ message: `Delivery sent OTP error ${error}` });
  }
};
